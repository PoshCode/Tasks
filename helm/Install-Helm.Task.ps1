    ...
    Jobs = {
        ...
        if (-not (Get-Command helm -ErrorAction SilentlyContinue)) {
            if ($IsLinux) {
                ...
                $TarFile = Join-Path $script:TempRoot "helm.tar.gz"
                Invoke-WebRequest -Uri "https://get.helm.sh/helm-${HelmVersion}-linux-amd64.tar.gz" -OutFile $TarFile
                tar -zxvf $TarFile -C $script:TempRoot
                Move-Item (Join-Path $script:TempRoot "linux-amd64/helm") "/usr/local/bin/helm" -Force
                Remove-Item $TarFile -Force -ErrorAction SilentlyContinue
                Remove-Item (Join-Path $script:TempRoot "linux-amd64") -Recurse -Force -ErrorAction SilentlyContinue
            } else {
