    ...
    Jobs = "Pack-Helm", "Connect-AzACR", {
        if ($script:PushEnabled) {
            foreach ($Chart in $script:HelmCharts) {
                ...
                $ChartToPush = Get-ChildItem (Join-Path $script:helmOutputRoot $Chart.Name) -Filter *.tgz | Sort-Object LastWriteTime | Select-Object -Last 1
