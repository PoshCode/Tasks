Add-BuildTask HelmPushChart @{
    ...
    Jobs = "HelmPackChart", "ConnectAzACR", {
        if ($BuildSystem -ne 'None' -and
            $BranchName -in "master", "main" -or
            $BranchName -like "release*" -or
            $BranchName -like "hotfix*") {
            ...
            foreach ($Chart in $script:HelmCharts) {
                ...
                Invoke-Native { helm push $ChartToPush.FullName "oci://$($script:ACRName).azurecr.io/helm" }
            }
